    ...
    if (file) {
      ...
      reader.onload = function (e) {
        ...
        runtime.sendMessage({
          ...
          prompt: 'Provide a concise, SEO-optimized description of this image in English, Spanish, Canadian French, and Portuguese. Format the response as a list with the language name as the heading for each description. Do not include the word "image" or any hashtags in any of the descriptions.',
          mimeType: file.type,
          imageData: imageDataUrl.split(',')[1]
        }, (response) => {
          console.log('Callback ejecutado. Respuesta:', response); // <-- Agrega esto

          if (!response) {
            outputContainer.innerHTML = '<p>No response from background script.</p>';
            return;
          }
          ...
          if (response.success) {
            ...
            if (data.candidates && data.candidates[0].content.parts.length > 0) {
              ...
              console.log('Texto combinado:', combinedText); // <-- Agrega esto
